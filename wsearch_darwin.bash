...
for F in *_R1.fq; do
	BASE=${F##*/}
	SAMPLE=${BASE%_*}
	$(./dependicies/vsearch_darwin --fastq_mergepairs $F --reverse ${SAMPLE}_R2.fq --fastqout $output/${SAMPLE}_merged.fq -relabel ${SAMPLE}. --threads $threads)
    $(./dependicies/falco_darwin -o $output/${SAMPLE}_falco_output $F ${SAMPLE}_R2.fq
...
if [[ "primer" == ""]]; then
    ...
    $(./dependicies/vsearch_darwin --fastq_filter $output/all_samples_merged.fq --fastq_stripleft $variable1 --fastq_stripright $variable2 -fastq_maxee 1 --fastq_qmax 42 --fastq_maxlen 290 --fastq_minlen 220 --fastaout $output/QCd_merged.fa)
...
$(./dependicies/vsearch_darwin --derep_fulllength $output/QCd_merged.fa -sizeout -relabel Uniq -output $output/uniques_vsearch.fa --threads $threads)
...
if [[ "$spe_def" == "ASV" ]]; then
    ...
    $(./dependicies/vsearch_darwin --usearch_global $output/QCd_merged.fa --db $output/ASVs.fa --id 0.99 --otutabout $output/ASV_counts.txt --threads $threads --strand both)
    ...
    if [[ "$spe_def" == "both" ]]; then
        ...
        $(./dependicies/vsearch_darwin --usearch_global $output/QCd_merged.fa --db $output/otus.fa --id 0.97 --otutabout $output/otu_counts.txt --threads $threads --strand both)
