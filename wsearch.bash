...
for F in *_R1.fq; do
	BASE=${F##*/}
	SAMPLE=${BASE%_*}
	$(vsearch --fastq_mergepairs $F --reverse ${SAMPLE}_R2.fq --fastqout ${SAMPLE}_merged.fq -relabel ${SAMPLE}. --threads $threads)
done
...
if [[ "$spe_def" == "ASV" ]]; then
    ...
    if [[ "$tax" == "NBC" ]]; then
        ...
        if [[ "$db" == "" ]]; then
            ...
            $(vsearch --sintax $output/ASVs.fa --db silva_16s_v123.fa --tabbedout $output/asv_tax_sintax.txt --threads $threads)
        else
            $(vsearch --sintax $output/ASVs.fa --db $db --tabbedout $output/asv_tax_sintax.txt --threads $threads)
        fi
    ...
    if [[ "$spe_def" == "both" ]]; then
        ...
        if [[ "$tax" == "NBC" ]]; then
            ...
            if [[ "$db" == "" ]]; then
                ...
                $(gunzip silva_16s_v123.fa.gz)
                $(vsearch --sintax $output/ASVs.fa --db silva_16s_v123.fa --tabbedout $output/asv_tax_sintax.txt --threads $threads)
                $(vsearch --sintax $output/otus.fa --db silva_16s_v123.fa --tabbedout $output/otu_tax_sintax.txt --threads $threads)
            else
                $(vsearch --sintax $output/ASVs.fa --db $db --tabbedout $output/asv_tax_sintax.txt --threads $threads)
                $(vsearch --sintax $output/otus.fa --db $db --tabbedout $output/otu_tax_sintax.txt --threads $threads)
            fi
        ...
        if [[ "$tax" == "NBC" ]]; then
            ...
            if [ "$db" == "" ]; then
                $(wget https://www.drive5.com/sintax/silva_16s_v123.fa.gz)
                $(gunzip silva_16s_v123.fa.gz)
                $(vsearch --sintax $output/otus.fa --db silva_16s_v123.fa --tabbedout $output/otu_tax_sintax.txt --threads $threads --sintax_cutoff 0.5)
            else
                $(vsearch --sintax $output/ASVs.fa --db $db --tabbedout $output/asv_tax_sintax.txt --threads $threads --sintax_cutoff 0.5)
            fi
...
if [[ "$tre" == "T" ]] ; then
    if ! [ -x "$(command -v mafft)" ]; then
        ...
        if [[ "$spe_def" == "ASV" ]] ; then
            $(mafft --thread $threads --auto $output/ASVs.fa $output/ASVs_aligned.fa)
            echo "multiple sequence alignment for ASVs done"
        else
            if [ "$spe_def" == "both" ] ; then
                $(mafft --thread $threads --auto $output/ASVs.fa $output/ASVs_aligned.fa)
                $(mafft --thread $threads --auto $output/otus.fa $output/otus_aligned.fa)
